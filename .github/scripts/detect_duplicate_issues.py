...
def similarity(target: dict, candidate: dict) -> float:
    ...
    # Hard veto (applied last, after any title boost): if both issues report
    # errors but share none of them, they are almost certainly different bugs
    # in the same area (e.g. "cannot find module ./config" vs "./database", or
    # two forms failing on different null references). Cap the score below the
    # decision threshold so near-identical titles/wording with a different
    # root error are not flagged as duplicates.
    if target["error_lines"] and candidate["error_lines"] and error_sim == 0.0:
        score = min(score, STRONG_MATCH * 0.9)
    return score
...
def find_duplicates(target: dict, candidates: Iterable[dict]) -> list[tuple[int, float]]:
    ...
    for cand in candidates:
        if cand["number"] == target["number"]:
            continue
        # similarity() already boosts strong title-only matches and vetoes
        # differing-root-error pairs, so a single score check is sufficient.
